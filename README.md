...
git clone --branch v2.9.0 --depth 1 https://github.com/Egonex-AI/Understand-Anything.git ~/.understand-anything/repo
...
```powershell
# Windows (PowerShell)
git clone --branch v2.9.0 --depth 1 https://github.com/Egonex-AI/Understand-Anything.git "$HOME\.understand-anything\repo"
...
> [!NOTE]
> `SHA256SUMS` is generated by `scripts/release-checksums.sh`, introduced in this same change — no release published before it will have that file, so `curl .../SHA256SUMS` 404s on any tag up through `v2.9.0`. Use the **first release published after this PR merges**, not `v2.9.0` itself. Until then, use the "Recommended" clone-based install above (git's own transport integrity covers you there — no separate checksum file needed).

```bash
# macOS / Linux — replace TAG with the first release published after this change merged
TAG=REPLACE_WITH_FIRST_RELEASE_AFTER_THIS_PR
...
```powershell
# Windows — replace $Tag with the first release published after this change merged
$Tag = 'REPLACE_WITH_FIRST_RELEASE_AFTER_THIS_PR'
