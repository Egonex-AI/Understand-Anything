...
| `domain-discoverer` | Идентификация бизнес-доменов и распределение модулей из сжатого KG-резюме (используется `/understand-domain`) |
| `domain-flow-extractor` | Извлечение бизнес-потоков и шагов для одного домена из его KG-подмножества (используется `/understand-domain`) |
| `domain-analyzer` | Устаревший монолитный доменный анализ (заменён разделением discoverer + flow-extractor) |
| `article-analyzer` | Извлечение сущностей, утверждений и неявных связей из статей вики (используется `/understand-knowledge`) |
| `wiki-worker` | Генерация wiki-контента для одного домена (используется `/understand-wiki`) |
| `wiki-reviewer` | Проверка качества wiki-вывода и соответствия схеме (используется `/understand-wiki`) |
| `assemble-reviewer` | Ревью собранных кросс-сервисных wiki-артефактов (используется `/understand-wiki`) |
| `upstream-updater` | Обнаружение изменений в upstream и запуск инкрементального повторного анализа (используется `/understand-wiki`) |
