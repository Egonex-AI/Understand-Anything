export const ja = {
  ...
  graphView: {
    focused: "フォーカス中",
    showingNeighborhood: "近接ノードを表示中",
    isolate: "関連のみ表示",
    isolateTitle: "このノードと直接接続されたノードのみ表示",
    resetView: "表示をリセット",
    resetViewTitle: "選択を解除してグラフ全体を表示",
  },
  projectOverview: {
    nodes: "ノード",
    edges: "エッジ",
  ...
  onboarding: {
    ...
    steps: [
      ...
      {
        title: "上部の 3 つのビュー",
        body: "概要は全体像（力学的レイアウト）、学習はあらかじめ用意された学習パス、詳細はタイプや複雑度の統計を表示します。それぞれ異なる問いに答えるためのビューです。",
        hint: "切り替える前に、何を知りたいかを明確に",
      },
      {
        title: "検索 + ノードクリック",
        body: "上部の検索ボックスはノード名、概要、タグをあいまい検索します。任意のノードをクリックすると、右側のパネルに概要、隣接ノード、「記事を開く」ボタンが表示されます。",
        hint: "検索はノードを中央寄せ・ハイライト、クリックは隣接エッジをハイライトします",
      },
      {
        title: "レイヤー切替とプロジェクトツアー",
        body: "上部の「すべて」の隣にあるレイヤータブは、index.md に基づいて 1 つのカテゴリだけを表示します。右側のプロジェクトツアーは、コードを理解しやすい順序で案内します。",
        hint: "ノードが多すぎるときはレイヤー、入り口がわからないときはツアー",
      },
      {
        title: "その他の隠れた機能",
        body: "上部バーにはフィルター（タイプや複雑度で絞り込み）、エクスポート（グラフを書き出す）、パス（2 つのノード間の経路を検索）、テーマ（表示テーマの切り替え）もあります。Shift + ? で全キーボードショートカットを確認できます。",
